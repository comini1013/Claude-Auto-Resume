...
### ⚠️ 꼭 읽어주세요! (Notice)
이 프로그램은 클로드의 내부 코드를 수정하거나 복잡한 API를 사용하는 '고급 개발 도구'가 아닙니다. 

저처럼 클로드와 대화하며 업무를 수행(Cowork)하는 일반 사용자가, **사용 한도 초과 시 발생하는 대기 시간의 불편함을 해소하기 위해 만든 단순한 UI 자동화 도구**입니다. 사용자의 컴퓨터에서 정해진 시간에 딱 한 번, 마우스 버튼을 대신 눌러주는 '친절한 비서' 역할을 수행할 뿐입니다.
...
본 프로그램은 사용자의 편의를 위한 교육 및 개인적 용도의 자동화 도구입니다. 
1. 본 프로그램의 사용으로 인해 발생하는 서비스 이용 제한 등의 모든 책임은 사용자 본인에게 있습니다.
2. 공식 API가 아닌 UI 자동화 방식을 사용하므로, 서비스 제공사(Anthropic)의 정책 변경에 따라 작동하지 않거나 약관 위반의 소지가 있을 수 있음을 알려드립니다.
